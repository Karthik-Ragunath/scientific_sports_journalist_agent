...
Records screen continuously and uploads segments to S3.
...
Usage:
    python screen_recorder.py --segment-duration 60
"""
...
class ScreenRecorder:
    """Records screen in segments - each segment is a separate ffmpeg run."""
    
    def __init__(self, output_dir: str, segment_duration: int = 60, 
                 fps: int = 30, quality: str = "medium", uploader: S3Uploader = None):
    ...
    def _get_output_filename(self) -> str:
        ...
        return str(self.output_dir / f"recording_{timestamp}_{self.segment_count:03d}.mp4")
    ...
    def _record_segment(self, output_file: str) -> bool:
        ...
        cmd = [
            ...
            "-capture_cursor", "1",
            "-i", "1:none",
            "-t", str(self.segment_duration),  # Time limit - this makes ffmpeg exit cleanly!
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-crf", crf,
            "-pix_fmt", "yuv420p",
        ...
        # Wait for ffmpeg to finish (either time limit reached or we stop it)
        try:
    ...
    def start(self):
        ...
        print(f"[RECORDER] Starting screen recording")
...
def main():
    default_output = os.path.expanduser("~/Downloads/dhivya/gemini-superbowl/screen_recordings")
    ...
    print("=" * 60)
    print("SCREEN RECORDER WITH S3 UPLOAD")
    print("=" * 60)
